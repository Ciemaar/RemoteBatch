...
## 3. Type Checker: `pyright` vs. `mypy`
...
- **Decision**: Adopt `pyright`. It provides a tighter integration with modern IDEs (like VSCode) and is faster for CI. To manage the strictness around PyQt6's optional returns, we use `typeCheckingMode = "basic"` in `pyproject.toml`, which provides a good balance between catching real bugs and avoiding false positives from untyped third-party libraries. We also removed `mypy` from the active checks to avoid split-brain configurations.
...
- **Testing**: `pytest` (with `pytest-cov` and `hypothesis`)
